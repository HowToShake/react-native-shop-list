...
const List = ({route: {params}}: {route: {params: ListType}}) => {
  ...
  const getProducts = () => {
    db.transaction(tx => {
      tx.executeSql(
        `SELECT * FROM lists WHERE listName LIKE '${params.listName}'`,
        [],
        (tx, resultSet) => {
  ...
  const setProductIsBought = async (id: string | number, isBought: boolean) => {
    await db.transaction(
      tx => {
        tx.executeSql(
          'UPDATE lists ' + `SET isBought=${isBought} ` + `WHERE id=${id};`,
        );
      },
      error => console.log('error', error),
      () => console.log('success'),
      ...
      <VStack space={4}>
        ...
        <VStack space={2}>
          {products
            ...
            ?.map((item: ListType, index: number) => (
              <HStack
                w="100%"
                ...
                <Checkbox
                  ...
                  <Box
                    ...
                    justifyContent="space-between"
                    alignItems="center">
                    <Text mx="2" strikeThrough={item.isBought}>
                      {item?.product}
                    </Text>
                    <Box flexDirection="column">
                      <Text strikeThrough={item.isBought}>
                        Amount: {item?.amount}
                      </Text>
                      <Text strikeThrough={item.isBought}>
                        Total Price:{' '}
                        {toNumber(item?.amount) * toNumber(item?.price)}
                      </Text>
                    </Box>
                  </Box>
                </Checkbox>
